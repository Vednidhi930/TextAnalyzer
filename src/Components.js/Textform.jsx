...
export default function Textform(props) {
  ...
  const convertCaptilize = () => {
    let captalize = text.split(" ");
    console.log(captalize);
    console.log(captalize.length);

    for (let i = 0; i < captalize.length; i++) {
      captalize[i] = captalize[i][0].toUpperCase() + captalize[i].substring(1);
    }
    console.log(captalize);
    setText(captalize.join(" "));
  };
  return (
    <>
      <h1
